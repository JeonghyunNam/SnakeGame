...
Controller::~Controller() {
    ...
    if(this->ptr_referee  != NULL)   delete ptr_referee;
...
void Controller::start() {
    ...
    while(1){
        ...
        if(_getch() == MAGICKEY){
            input = _getch();
            if(input != UP && input == DOWN && input == LEFT && input == RIGHT) continue;
        }
        else    continue;
